...
- **flickr_client.py** — Wraps `flickrapi.FlickrAPI` in JSON mode. Uses `extras` parameter on `getPhotos` to batch-fetch photo metadata (avoids per-photo API calls, respects 3,600 req/hr rate limit). Requests multiple image size URLs (url_k=2048px, url_h=1600px, url_l=1024px, url_c=800px, url_z=640px, url_m=500px, url_o=original) so the downloader can pick based on `FLICKR_IMAGE_SIZE` config. Handles Flickr's `{"_content": "..."}` nesting pattern via `_text()` helper.
- **models.py** — Three Pydantic models: `ExhibitionMetadata` (LLM output type), `PhotoRecord`, `AlbumRecord`. The `AlbumRecord` composes the other two. `PhotoRecord` stores URLs for multiple image sizes (2048px, 1600px, 1024px, 800px, 640px, 500px, original).
...
- **downloader.py** — Async httpx with semaphore concurrency. Selects image size based on `FLICKR_IMAGE_SIZE` config (defaults to `large` = 1024-2048px). Falls back through URL sizes if preferred size unavailable. Skips existing files for idempotent re-runs.
